...
const InviteTable = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const response = await fetch(`${process.env.REACT_APP_API_URL}/getInvites`, {
					method: 'POST',
					headers: {
	...
	const acceptInvite = async () => {
		const response = await fetch(`${process.env.REACT_APP_API_URL}/acceptInvite`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
		...
		if (json.success) {
			if (json.newAuthToken) localStorage.setItem('authToken', json.newAuthToken);
			navigate('/trip', { state: { tripid: currTrip._id } });
	...
	const rejectInvite = async () => {
		const response = await fetch(`${process.env.REACT_APP_API_URL}/rejectInvite`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
