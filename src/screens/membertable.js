...
const MemberTable = ({ tripid, ownerId }) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const response = await fetch(`${process.env.REACT_APP_API_URL}/getTripMembers`, {
					method: 'POST',
					headers: {
	...
	const confirmKick = async () => {
		const response = await fetch(`${process.env.REACT_APP_API_URL}/kickMember`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
	...
	const confirmCAdmin = async () => {
		const response = await fetch(`${process.env.REACT_APP_API_URL}/adminMember`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
